...
class BoardTest_Sprint3 {
    ...
    void placingOnOccupiedCellRejected() {
        assertTrue(b3.place(0,0, sprint_3.Move.S));
        assertFalse(b3.place(0,0, sprint_3.Move.O));
        assertEquals(sprint_3.Move.S, b3.getCell(0,0));
    }

    @Test
    void outOfBoundsPlacementRejected() {
        assertFalse(b3.place(-1,0, sprint_3.Move.S));
        assertFalse(b3.place(0,-1, sprint_3.Move.O));
        assertFalse(b3.place(3,0, sprint_3.Move.S));
        assertFalse(b3.place(0,3, sprint_3.Move.O));
    }
    ...
    void gridFullDetection() {
        for (int r = 0; r < b3.getSize(); r++) {
            for (int c = 0; c < b3.getSize(); c++) {
                assertTrue(b3.place(r, c, sprint_3.Move.S));
            }
        }
